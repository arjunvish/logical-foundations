...
Proof.
  intros x l. induction l.
  + simpl. reflexivity.
  + simpl. rewrite rev_app_distr. rewrite IHl. simpl.
    reflexivity.
Qed.
(** [] *)
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => (ltb 7 n)) (filter (fun n => evenb n) l).
...
Theorem map_app : forall (X Y: Type) (f : X -> Y) (l m : list X),
                map f (l ++ m) = (map f l) ++ (map f m).
Proof.
  intros X Y f l m. induction l.
  + simpl. reflexivity.
  + simpl. rewrite IHl. reflexivity.
Qed.
...
Proof.
  ...
  + simpl. reflexivity.
  + simpl. rewrite map_app. rewrite IHl. simpl.
    reflexivity.
Qed.
(** [] *)
...
Fixpoint flat_map_aux {Y : Type} (l : list (list Y)) : (list Y) :=
  match l with
  | nil => nil
  | h :: t => h ++ flat_map_aux t
  end.

Fixpoint flat_map {X Y: Type} (f: X -> list Y) (l: list X)
                   : (list Y) :=
  flat_map_aux (map f l).
...
(* FILL IN HERE *)
Theorem fold_map_correct : forall (X Y : Type) (f : X -> Y) 
  (l : list X), map f l = fold_map f l.
Proof.
  intros X Y f l. induction l.
  + reflexivity.
  + simpl. rewrite IHl. reflexivity.
