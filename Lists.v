...
Proof.
  intros p. destruct p. reflexivity. Qed. 
(** [] *)
...
Fixpoint count_aux (v:nat) (s:bag) (acc:nat) :=
  match s with
  | nil => acc
  | h :: t => if (eqb h v) then 
                count_aux v t (S acc)
...
(* Definition *) Fixpoint member (v:nat) (s:bag) : bool :=
  match s with
  | nil => false
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | nil, s2 => true
  | s2, nil => false
  | h1 :: t1, h2 :: t2 => if (member h1 s2) then 
...
Theorem bag_theorem : forall (s : bag), sum s [] = s.
Proof.
  induction s.
  + reflexivity.
...
Proof. 
  ...
  + reflexivity.
  + simpl.
    assert (H : forall (m : nat), m =? m = true).
    { induction m.
      + reflexivity.
